...
public class GorillaPursueState : BaseState
{
    ...
    {
        if (Physics2D.OverlapBox(_myGorilla.transform.position, Vector2.one, 0, _myGorilla._playerMask) && !isAttacking)
        {
            isAttacking = true;
        ...
        if (!isAttacking)
        {
            _myGorilla.Move(-Mathf.Sign(_myGorilla.transform.position.x - _character.transform.position.x));
        }
        else
        {
            if ((Mathf.Sign(newNode.transform.position.x - _myGorilla.transform.position.x) > 0))
            {
                _myGorilla.Move(-Mathf.Sign(_myGorilla.transform.position.x - (_character.transform.position.x - _myGorilla.attackDistance)));
